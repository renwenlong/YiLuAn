...
#
# Scenarios:
#   1. all 4 required success            → exit 0 (PASS)
...
#   6. extra non-required check red,     → exit 0 (PASS — only required gate)
#      all required green
#   7. malformed JSON                    → exit 2 (env error)
...
# 6. extra non-required red, all required green → PASS (only required gate)
make_fixture "${TMPDIR_T}/extra_red.json" \
  "${R1}=success" "${R2}=success" "${R3}=success" "${R4}=success" \
  "Some Optional Check=failure" "Notify on CI failure=skipped"
assert_exit "extra non-required red, required all green → PASS" 0 "${TMPDIR_T}/extra_red.json"

# 7. malformed JSON → env error (exit 2)
