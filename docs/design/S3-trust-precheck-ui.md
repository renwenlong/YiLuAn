...
| ABAC counter | backend `precheck_abac_filtered_total` Prometheus counter；ADR-0056 明确双 scope：v1.0 `{endpoint,user_role,filter_reason}` owner-gate 已实施（PR #319），v2.0 `{card,field}` 字段级业务 dashboard deferred（触发条件：**UI 4 张牌 trust/precheck dashboard 立项时**）。不强求三端各自上报，client-side ABAC 不是设计 trust boundary；灰度告警规则见 `infra/prometheus/rules/precheck_abac.yml`。 |
