...
- **r5（2026-06-08）**：胡桃 S3-DEV-001-CONTRACT-SERVICE-CORE implement 起手前 fact check 拦下 3 个 implement gap，架构师 (魈) 拍 design amend
  ...
  - **EVENT-WIRING 触发点补发 (2026-06-08 08:35 UTC)**: 架构师 (魈) 08:18 UTC 拍 “单 accept_order hook” 设计被撤回，最终定为 **双 hook (payment.handle_pay_callback 主路径 + accept_order 兜底路径)**，与 ADR-0047 §3 line 257-258 `payment.succeeded → contract.generate.requested` 主路径一致。撤回理由: (1) ADR-0047 §3 多处明示 payment.succeeded 触发，架构师 08:18 UTC 凭业务约束推理错过该证据；(2) `create_prepay` backend grep 确认不要求 `status=accepted` 也不要求 `companion_id` 非 NULL，UI 约束 (patient 看不到未 accept 单的 pay button) ≠ schema 约束，P2P 模式 patient 先付后接单路径 schema 允许。`ContractService.request_generation(order_id) → Optional[ServiceContract]` 内部 eligibility check (companion_id 是否 NULL)，NULL 时返 None 跳过，非 NULL 时 INSERT pending_generation。双 hook 任一触发均安全 (UNIQUE order_id constraint 兼 IntegrityError catch 防双 INSERT)。设计化点: `request_generation` 返 Optional + 双 hook + UNIQUE 兜底 = **deep defense**, ADR-0041 PaymentState vs OrderStatus 双状态机解耦问题 (胡桃 docstring hold flag 指出) 未来业务路径变化时均有着陆点。架构师本次连踩两次错误 (08:18 + 08:32)，留痕作 OPS-021 (a) case study 第四章节 "架构师推理 vs 文档 evidence 冲突"
