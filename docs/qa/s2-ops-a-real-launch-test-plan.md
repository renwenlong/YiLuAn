...
### AC3 (AC-9): 24h 验收门槛
- ✅ 放行生效 (白名单真号放行绝对次数 > 0, 不依赖比率; ==0 持续 4 小时 = 切量未生效告警)
...
> 魈 2026-06-18 拍 (技术项写死), ③ N=4h 凝光 2026-06-18 拍定 (业务门槛)。
...
| ③ 白名单放行缺失 | **白名单用户放行绝对次数 == 0 持续 4 小时** (替代不稳的「放行率<X%」) | 排查切量未生效 / gate 误拦真白名单 | 魈定指标方向, **N=4h 凝光 06-18 拍定 (业务: 4 小时无放行算异常)** |

> **③ 指标选型说明** (魈): 放行率 = 放行/(放行+拦截), 分母是白名单用户真实访问量, 5-10 人下比率本身不稳 (一用户多刷拉高)。改测「放行绝对次数==0 持续 4 小时」不依赖业务量, 更稳。N=4h 是凝光拍定的业务预期 (白名单 4 小时不访问算异常)。

> **③ N=4h 为何不更短 (凝光 3 条业务理由, board comment `96050e2e-9e4c-49b0-9add-992819658002` 落痕)**:
> 1. **真人慢启动现实**: 5-10 真用户 24h 灰度, 分享路径非切量后立即用, 2h 内零放行很可能是真人没碰、非 gate 误拦 → 阈值太短会误报。
> 2. **双时间尺度互补**: fail-closed 触发即告警(①,魈拍)是【即时哨兵】, 4h 零放行是【慢病侦测】, 两者覆盖不同故障时间尺度, 不重叠。
> 3. **回滚 5min 可操作兼顾**: 即使 4h 后才告警, 回滚 <2min (关 flag), 告警→人工查→关 flag 总响应 ≤6h, 24h 灰度窗内仍可挽回。
>
> **③ 告警文案 hint (凝光建议, 省人工二次诊断)**: 零放行告警触发时, 文案带「先查 status_class 4xx 是否有 `SHARE_F2_CANARY_NOT_WHITELISTED` 突增」——有突增=gate 误拦真白名单(真问题), 无突增=真人慢启动(可等)。帮值班快速区分。
...
## 6. 当前状态 & 下一步
- **预案就绪 + 告警阈值全定稿 (魈拍①② 06-18, 凝光拍③ N=4h 06-18)**。
