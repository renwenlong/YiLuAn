...
_PLACEHOLDER_PARAGRAPHS: list[tuple[str, str]] = [
    (
        "1. 服务范围",
        "乙方按订单约定为甲方(或代订对象)提供陪诊相关服务,包括但不限于挂号、\n"
        "就诊陪同、取药、与医务人员沟通协助等。具体内容以订单服务项目为准。\n"
        "[v1.0.0 草稿,等待 PM (凝光) 正式法务措辞]",
    ),
    (
        "2. 退款规则",
        "服务开始前申请退款的,按平台退款政策处理。服务开始后或服务完成的,\n"
        "原则上不予退款,但因乙方原因未能完成服务的,甲方可申请相应退款。\n"
        "[v1.0.0 草稿,等待 PM 正式细则]",
    ),
    (
        "3. 免责边界",
        "乙方不构成医疗服务提供者,不对医疗结果承担责任。乙方仅协助甲方就医,\n"
        "医疗决策与医嘱执行由甲方自行负责。\n"
        "[v1.0.0 草稿,等待 PM 正式免责措辞]",
    ),
    (
        "4. 个人信息处理(PIPL)",
        "平台依据《个人信息保护法》处理甲乙双方个人信息,详见《一路安隐私政策》。\n"
        "合同哈希采用 SHA-256 算法,患者姓名、身份证后四位等敏感字段以伪名化\n"
        "(salted SHA-256)形式参与哈希计算,本合同 PDF 不存储任何敏感明文。\n"
        "[v1.0.0 草稿,等待 PM / 法务最终措辞]",
    ),
]
